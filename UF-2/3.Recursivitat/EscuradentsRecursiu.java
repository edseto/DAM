...
public class EscuradentsRecursiu{
    ...
    public static int NomJugadors(String[] nomJugadors, int numJugadors){
        Scanner sc = new Scanner(System.in);
        int index;
        ...
        if(numJugadors==0){
            ...
            index = NomJugadors(nomJugadors, numJugadors-1);
            index++;
            System.out.print("Introdueix el nom del jugador"+index+": ");
            nomJugadors[index] = sc.next();
        }
        return numJugadors;
    }
    ...
    public static int Jugar(String[] nomJugadors, int numEscuradents, int numJugadors, String[] historial){
        Scanner sc = new Scanner(System.in);
    ...
    public static int Historial(String[] historial, int nMoves){
        int index;

        if(nMoves==0){
            return 0;
        }else{
            index = Historial(historial, nMoves-1);
            System.out.println(historial[index+1]);
        }
        return nMoves;
    }
    public static void main(String[] args) {
        ...
        char opcio;
        int numJugadors = 2, numEscuradents = 23, nMoves=0;
        ...
        do {
            ...
            switch (opcio){
                ...
                case 'c': numEscuradents = DemanarEscuradents();
                    break;
                case 'd': nMoves = Jugar(nomJugadors, numEscuradents, numJugadors, historial);
                    break;
                case 'e': Historial(historial, nMoves);
                    break;
                case 'f': finish = true;
                    break;
            }
